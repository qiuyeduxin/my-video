              ...
              <div class="player-tab-lyric">
                  <div class="player-tab-lyric-c" v-if="!lyricError">
                     <div class="player-tab-lyricList fs36" :style="moveStep">
                        <div class="player-tab-lyric-item texthidden" :class="{ 'active': $index == currentIndex }" v-for=" item in lyric">{{ item[1] }}</div>
                     </div>
                  </div>
...
export default {
  ...
  ready () { 	
    media('audio',{
        ...
        playElm: 'playElm',
        preload: 'load',
        src: this.PlayData.sourceUrl,
        lyricCallback: ( time ,endTime )=>{
          if( this.lyric.length != 0 ){
              this.lyric.forEach((item, index)=>{
                if(  Number(time).toFixed(3) > item[0] ){
                    this.currentIndex = index;
                    this.currentStep = -( index - 3 );
                }else if( Number(time).toFixed(3) <= this.lyric[0][0]){// sing start
                  this.currentIndex = -1;
                  this.currentStep =  3;
                }
              });
    ...
    //获取歌词
    this.$http.get(this.PlayData.lyricUrl).then((rep) => {
  ...
  computed:{
    moveStep (){
      return {
        'margin-top': ( this.currentStep * 1.4 ) + 'rem'
      }
    }
  },
  methods: {
...
.player-tab-lyric-item.active{color: #ffbb33; font-weight: bold; font-size: 0.4rem;}
